...
class TileNet(nn.Module):
    ...
    def triplet_loss(self, z_p, z_n, z_d, margin=0.1):
    ...
    def loss(self, patch, neighbor, distant, margin=0.1):
        """
        Computes loss for each batch.
        """
        z_p, z_n, z_d = (self.encode(patch), self.encode(neighbor),
            self.encode(distant))
        return self.triplet_loss(z_p, z_n, z_d, margin=margin)


class TileNetNoise(TileNet):
    def __init__(self, num_blocks, in_channels=4, z_dim=512):
        super().__init__(num_blocks, in_channels, z_dim)

    '''
    def loss_with_noise(self, patch, neighbor, distant, noise, margin=0.1, l2=l2, alpha=1.0, triplet=False):
        z_p, z_n, z_d = (self.encode(patch), self.encode(neighbor), self.encode(distant))
        
        tile2vec_loss = self.triplet_loss(z_p, z_n, z_d, margin=margin, l2=l2)
        noise_loss = noise_loss(z_p, z_n, z_d, noise, margin=0.1, l2=l2, triplet=triplet)

        return tile2vec_loss + alpha * noise_loss
    '''

    def noise_loss(self, z, noise, alpha=0.01):
        loss = alpha * torch.mean(torch.square(z - noise))
        return loss
...
def make_tilenet_noise(in_channels=4, z_dim=512):
    num_blocks = [2, 2, 2, 2, 2]
    return TileNetNoise(num_blocks, in_channels=in_channels, z_dim=z_dim)
